...
def test_load_into_default(
    ...
    if logging.getLogger().isEnabledFor(logging.DEBUG):
        debug_format = (
            "nquads" if isinstance(expected_graph, Dataset) else "ntriples"
        )
        logging.debug(
    ...
    if isinstance(expected_graph, Dataset):
        assert isinstance(actual_graph, Dataset)
...
def test_load_into_named(
    graph_factory: Callable[[], Dataset], source: GraphSource
) -> None:
    """
